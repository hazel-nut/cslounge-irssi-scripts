...
# New in latest version: msqrdstr now colorizes properly
...
my @rainbow = qw/4 8 9 11 12 13 4 8/;
...
sub sig_public {
	...
	if($level==2 && $msg=~/^[a-zA-Z0-9_|^\[\]]*[:,] /)
	{
		...
		my $inick=$1;
		my $rest=$2;
		if($inick eq "msqrdstr")
		{
		    my @chars=split(//,$inick);
		    my $colorized="";
		    foreach my $i ( 0 .. $#chars )
		    {
			$colorized = $colorized.chr(3).$rainbow[$i].$chars[$i];
		    }
		    $colorized = $colorized.chr(15);
		    Irssi::signal_emit('message public',
				       ($server, $colorized.$rest, $nick,
					$address, $target));
		    Irssi::signal_stop();
		    return;
		}
